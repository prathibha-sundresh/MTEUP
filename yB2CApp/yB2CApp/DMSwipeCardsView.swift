...
extension DMSwipeCardsView: DMSwipeCardDelegate {
	func cardSwipedLeft(_ card: DMSwipeCard) {
		...
		DispatchQueue.main.asyncAfter(deadline: .now() + 0.001) {
			self.delegate?.swipedLeft(card.tag)
			self.loadNextCard()
		}
	}
	...
	func cardSwipedRight(_ card: DMSwipeCard) {
		self.handleSwipedCard(card)
		DispatchQueue.main.asyncAfter(deadline: .now() + 0.001) {
			self.delegate?.swipedRight(card.tag)
			self.loadNextCard()
		}
	}

  func cardTapped(_ card: DMSwipeCard) {
    self.delegate?.cardTapped(card.tag)
  }
  
  func cardStartedDragging(_ card: DMSwipeCard) {
    self.delegate?.draggingStarted()
  }
  
  func cardStopedDragging(_ card: DMSwipeCard) {
    self.delegate?.draggingStopped()
  }
}
